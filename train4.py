...
if args.train:
    # weights = None
    weights = np.load('models/yolo_N4.npy', allow_pickle=True).item()
    ...
    weights = np.load('models/yolo_N4_bn.npy', allow_pickle=True).item()
    dropout = False
...
# 240, 288
model = model(layers=[
conv_block((7,7,4,64), 1, weights=weights, train=train_flag), # 240, 288
...
def extract_fn(record):
    ...
    image = tf.reshape(image, (240, 288, 4))
...
if args.train: filenames = collect_filenames('./dataset/train4')
else:          filenames = collect_filenames('./dataset/val4')
