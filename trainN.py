...
if args.train:
    # weights = None
    weights = np.load('models/z.npy', allow_pickle=True).item()
...
# 240, 288
model = model(layers=[
conv_block((7,7,args.frames,64), 1, weights=None, train=train_flag), # 240, 288
...
def extract_fn(record):
    ...
    image = tf.reshape(image, (240, 288, 16))
    image = image[:, :, -args.frames:]
